      ...
      <div className="order-table">
        <table>
          ...
          {allHoldings.map((stock, index) => {
            ...
            const avgValue = Number(stock.avg || 0) * Number(stock.qty || 0);
            const pnl = curValue - avgValue;
            const isProfit = pnl >= 0;
            const profClass = isProfit ? "profit" : "loss";
            const dayIsLoss = typeof stock.day === "string"
              ? stock.day.trim().startsWith("-")
              : Boolean(stock.isLoss);
            const dayClass = dayIsLoss ? "loss" : "profit";
